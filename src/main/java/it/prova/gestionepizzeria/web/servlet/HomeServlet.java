package it.prova.gestionepizzeria.web.servlet;
...
import it.prova.gestionepizzeria.web.filter.AuthenticationFilter;
...
public class HomeServlet extends HttpServlet {
	...
	private RequestDispatcher getRequestDispatcherIfUserLoggedInOrNot(HttpServletRequest request) {
		...
		if (AuthenticationFilter.isUserLoggedIn(request)
				&& AuthenticationFilter.getUserLoggedIn(request).isPizzaiolo()) {
			return request.getRequestDispatcher("/pizzaiolo/indexPizzaiolo.jsp");
		}
		if (AuthenticationFilter.isUserLoggedIn(request)
				&& AuthenticationFilter.getUserLoggedIn(request).isFattorino()) {
